...
<?
	...
	*/
	...
	$sql = "SELECT p.* FROM article_image p";
	$sql.= " WHERE 1=1 ";
	...
	for ($i=0;$i<$CntRecInPage;$i++) {
		...
		$insert["article_id"] 		= "'".$Row["article_id"]."'";
		
		$insert["file_name"] 		= "'".$Row["article_image_large"]."'";
		$insert["physical_name"] 	= "'".$Row["article_image_large"]."'";
		$extension 	= 	strtolower(strrchr($Row["article_image_large"], '.'));
		...
		@copy($libraryFolder."/".$Row['article_image_large'],$storeFolder."/".$Row['article_image_large']);
		
		$physical_name=$libraryFolder."/".$Row['article_image_large'];
		
		if ((is_file($physical_name) && file_exists($physical_name) && trim($Row['article_image_large'])!=''  )) {
			$resizeObj 	= new ResizeImage($libraryFolder."/".$Row['article_image_large']);
			$resizeObj->resizeTo(290,290,'maxwidth');
			$resizeObj->saveImage(_SYSTEM_UPLOADS_FOLDER_."/article_thumb/".$Row['article_image_large']);
