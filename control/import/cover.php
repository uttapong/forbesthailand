...
<?
	...
	$sql = "SELECT p.* FROM cover_main p";
	$sql.= " WHERE 1=1 ";
...
<?
	...
	if (!(is_file($physical_name) && file_exists($physical_name) )) {
		$physical_name="../img/photo_not_available.jpg";
			...
			//$Large = $_Config_live_site."/uploads/images/".$row["cover_image"];
			
			/*
			copy($libraryFolder."/".$Row['cover_image'],$storeFolder."/".$Row['cover_image']);
			
			$resizeObj 	= new ResizeImage($libraryFolder."/".$Row['cover_image']);
			$resizeObj->resizeTo(290,290,'maxwidth');
			$resizeObj->saveImage(_SYSTEM_UPLOADS_FOLDER_."/cover_thumb/".$Row['cover_image']);
			
			*/
			
			//	copy($libraryFolder."/".$Row['cover_image_thumb'],$storeFolder."/".$Row['cover_image_thumb']);
			
			
	$update="";	
	//$update[] = "filepic 		= '".$Row['cover_image']."'";
	//$update[] = "issue_en 		= '".$Row['issue_th']."'";
	
	//UPLOADPIC
	/*
	$update[] = "flag_media 		= 'UPLOADPIC'";
	$update[] = "filepiccontent 	= '".$Row['cover_image_thumb']."'";
	*/
	///uploads/userfiles
	...
	//echo str_replace('/uploads/userfiles/',_SYSTEM_HOST_NAME_.'/uploads/userfiles/',$Row['content_th']);
	
	//$update[] = "content_th 		= '".$Row['name_th']."'";
	//$update[] = "content_en 		= '".$Row['content_th']."'";
	//$update[] = "keyword_en 		= '".$Row['keyword_th']."'";
	//	$update[] = "approvedby 	= 'admin'";
	//	$update[] = "createby 		= 'admin'";
	//	$update[] = "updateby 		= 'admin'";

	//$update[] = "cate1_id 		= '".$Row['cate_id']."'";
