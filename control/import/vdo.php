...
<?
	...
	if ($SysFSort=="") $SysFSort="p.video_id";
	...
	*/
	...
	// ID LAST ARTICLE =>  890 , LEADER(919)  =>    
	...
	for ($i=0;$i<$CntRecInPage;$i++) {
		...
		$insert["menu_id"] 				= "'MNT14'";	
		$insert["cover_id"] 			= "'".$Row["cover_id"]."'";
		
		$insert["cate_id"] 				= "'33'"; // THAILAND
		$insert["cate1_id"] 			= "'33'"; // THAILAND		

		$insert["name_th"] 			= "".SystemCheckInputToDB($Row["video_title"])."";
		$insert["name_en"] 			= "".SystemCheckInputToDB($Row["video_title"])."";
		$insert["abstract_th"] 		= "".SystemCheckInputToDB($Row["video_abstract"])."";
		$insert["abstract_en"] 		= "".SystemCheckInputToDB($Row["video_abstract"])."";	
		
		
		$insert["content_th"] 			= "".SystemCheckInputToDB($Row["video_detail"])."";
		$insert["content_en"] 			= "".SystemCheckInputToDB($Row["video_detail"])."";
		$insert["article_type"] 		= "'".$Row["video_type"]."'";
		...
		$insert["filepic"] 			= "'".$Row['video_image_thumb']."'";
		$insert["filepiccontent"] 			= "'".$Row['video_image']."'";
		
		$insert["flag_media"] 		= "'EMBED'";
		$insert["embed_code"] 	= "'".$Row['video_embed']."'";	
		$insert["embed_vdo"] 	= "'".$Row['video_embed']."'";	
		
	
		$storeFolder = _SYSTEM_UPLOADS_FOLDER_.'/article';  
		$libraryFolder = _SYSTEM_UPLOADS_FOLDER_.'/images'; 
	 /* 
		@copy($libraryFolder."/".$Row['video_image_thumb'],$storeFolder."/".$Row['video_image_thumb']);
		
		$physical_name=$libraryFolder."/".$Row['video_image_thumb'];
		
		if ((is_file($physical_name) && file_exists($physical_name) )) {
		
			$resizeObj 	= new ResizeImage($libraryFolder."/".$Row['video_image_thumb']);
			$resizeObj->resizeTo(290,290,'maxwidth');
			$resizeObj->saveImage(_SYSTEM_UPLOADS_FOLDER_."/article_thumb/".$Row['video_image_thumb']);
			
		}
		
		@copy($libraryFolder."/".$Row['video_image'],$storeFolder."/".$Row['video_image']);
		...
		$insert["cstate"] 			= "'".$Row["visited"]."'";
		$insert["order_by"] 			= "'".$Row["video_order"]."'";
