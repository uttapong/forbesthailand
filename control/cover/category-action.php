...
if($ModuleAction == "AddForm"){
	...
	$sql = "select *  from cover_category where cate_id = '$ModuleDataID'";
...
<label class="control-label" for="category_name_en">Category name (EN)</label>
<div class="controls">
<input type="text" id="category_name_en" name="category_name_en" class="request" value="<?=$Row["name_en"]?>" required>
</div>
</div>

<div class="control-group">
<label class="control-label" for="category_name_th">Category name (TH)</label>
<div class="controls">
<input type="text" id="category_name_th" name="category_name_th" class="request" value="<?=$Row["name_th"]?>" required>
</div>
</div>
...
<?
		...
		$input_name_th = SystemCheckInputToDB($_REQUEST["category_name_th"]);
		$input_name_en = SystemCheckInputToDB($_REQUEST["category_name_en"]);
		
		$insert["name_en"] 				= "".$input_name_th."";
		$insert["name_th"] 				= "".$input_name_en."";
		...
		$_order_by=SystemGetMaxOrder("cover_category","site_code='S0001'")+1;
		$insert["order_by"] 			= "'".$_order_by."'";
		
		$sql = "insert into cover_category(".implode(",",array_keys($insert)).") values (".implode(",",array_values($insert)).")";
		$Conn->execute($sql);
...
<? 
	...
	$sql = "update  cover_category set ".implode(",",array_values($update)) ;
...
<?
	...
	for($i=0;$i<$max; $i++){
		...
		$sql="UPDATE cover_category SET ".implode(",",$update)." WHERE cate_id='".$TmpArrID[$i]."' ";
		$Conn->execute($sql);
