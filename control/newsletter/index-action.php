...
if ($ModuleAction == "Datalist") {
	...
	$sql = "SELECT p.* FROM newsletter_main p";

	$sql.= " WHERE 1=1 ";
	
	
	if (trim($SysTextSearch)!=""){
		$sql.=" AND (p.email  like '%".$SysTextSearch."%' ";
		$sql.=")";
	}
...
<a href="export.php?<?=SystemEncryptURL("SysMenuID=$SysMenuID&ModuleAction=AddForm")?>" target="_blank" class="btn btn-success" ><i class="icon-plus"></i> Export</a>
...
<th <?=SysGetTitleSort('p.name',$SysFSort,$SysSort)?>   >Email</th>

<th <?=SysGetTitleSort('p.flag_display',$SysFSort,$SysSort)?> style="width:225px;" >Register Date</th>
...
<td class="" valign="top"><?=$Row["email"]?></td>
<td class="span2" style="text-align:center;" ><?=$Row["createdate"]?> </td>
...
$pid= $_REQUEST["id"];
$sql = "delete from newsletter_main where id = '".$pid."'";
$Conn->execute($sql);
