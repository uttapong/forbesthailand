...
if ($ModuleAction == "Datalist") {
	...
	$sql = "SELECT p.*,c.name_".strtolower($_SESSION["LANG"])." as category_name,u.firstname,u.lastname FROM article_main p";
...
-->
<? my_loadTreeCatSelect(0, $SysCateID); ?>
</select> 
...
<a href="index.php?<?=SystemEncryptURL("SysMenuID=$SysMenuID&ModuleAction=Sorting&SysCateID=$SysCateID")?>" class="btn" ><i class="icon-list"></i> Sorting</a>
...
<thead>
<tr role="row">
<th style="width:50px;" <?=SysGetTitleSort('p.order_by ',$SysFSort,$SysSort)?>  >ลำดับ</th>
...
<tr >
<td class="span1 text-center" style="text-align:center; font-size:24px; font-weight:bold;"><?=($Row["order_by"]+1)?> </td>
...
<?
	...
	if($input_fileIDOnePic!=""){
		...
		$input_value_th = SystemCheckInputToDB($_REQUEST["input_value_th"]);
		$input_business_th = SystemCheckInputToDB($_REQUEST["input_business_th"]);
		...
		$input_value_en = SystemCheckInputToDB($_REQUEST["input_value_en"]);
		$input_business_en = SystemCheckInputToDB($_REQUEST["input_business_en"]);
		...
		$insert["value_th"] 			= "".$input_value_th."";
		$insert["business_th"] 			= "".$input_business_th."";
		
		$insert["name_en"] 				= "".$input_name_en."";
		$insert["content_en"] 			= "".$input_content_en."";
		$insert["keyword_en"] 			= "".$input_keyword_en."";
		$insert["value_en"] 			= "".$input_value_en."";
		$insert["business_en"] 			= "".$input_business_en."";
		...
		$_order_by = SystemGetMaxOrder("article_main","menu_id='".$_REQUEST['SysMenuID']."' and cate_id='".$_REQUEST['cate_id']."'")+1;
...
<? 
	...
	$update[] = "value_th 			= ".$input_value_th."";
	$update[] = "business_th 		= ".$input_business_th."";
	
	$update[] = "name_en 			= ".$input_name_en."";
	$update[] = "content_en 		= ".$input_content_en."";
	$update[] = "keyword_en 		= ".$input_keyword_en."";
	$update[] = "value_en 			= ".$input_value_en."";
	$update[] = "business_en 		= ".$input_business_en."";
