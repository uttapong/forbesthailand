...
$file_name= 	$_FILES['file']['tmp_name'];  
	...
	if($extension!=".gif"){
		...
		if($_REQUEST["ObjUFileResizeType"]=="CROP"){
			$resizeObj->resizeTo($_REQUEST["ObjUFileResizeWidth"], $_REQUEST["ObjUFileResizeHeight"], 'crop');
		}else{
			$resizeObj->resizeTo(1100,1460);
		}
		...
		$physical_name	= $_FILES['file']['name'];
		$physical_name 	= md5($_FILES['file']['name'].date("His").rand(1,1000)).$extension;
		$targetFile =  $targetPath.$physical_name;
	...
	echo "<script>parent.getSelectImg".$ObjUFileID."('$FileID');</script>";
}
