...
function getSelectImg(_id,_type){
	...
	if(_class.indexOf("dz-success")=="-1"){
		...
		$.ajax({
			...
			data : "ModuleAction=getFileUploadAll&file_id="+_id,
			type : "post",
			cache : false ,
...
$(document).ready(function() {
	$("#file-upload-all").dropzone({ 
	   url: "../obj/file-upload/upload-all.php" ,
...
<div class="tabbable" >
    <ul class="nav nav-tabs" id="libraryTabs" style="font-size:20px;" >    
        ...
        <li><a href="#library" data-toggle="tab">เลือกจากคลังไฟล์</a></li>
    </ul>
 
    <div class="tab-content" >
        <div class="tab-pane active" id="newupload" style="height:400px;">
        <form action="../obj/file-upload/upload-one.php" target="up_iframe"  method="post" enctype="multipart/form-data" id="file-upload-all"  class="dropzone" style="min-height:360px;">
    ...
    $(function() {
		$('#libraryTabs').bind('show', function(e) { 			
		   ...
		   if(contentID=='#library'){
				var Vars=$('#file-select-all').serialize();
...
<form id="file-select-all" style="margin:0" >
...
function library_ok_select(){
	var Vars=$('#file-select-all').serialize();
	 window.parent.library_feedFileUploadAll(Vars);
}
</script>
<a class="btn"  href="javascript:iframe_closeModal();">Close</a>
<button type="submit" class="btn btn-success" onclick="library_ok_select();"><i class="icon-ok"></i> Insert Fiels</button>
